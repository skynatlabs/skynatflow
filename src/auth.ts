...
// Google credentials come from the database first (settable at
// /admin/api-keys, no redeploy), falling back to env vars — same
// DB-then-env pattern as src/lib/ai/model.ts. Building the config as a
// function (Auth.js v5 supports this) instead of a static object is what
// lets this be checked fresh per request rather than baked in at boot.
...
export const { handlers, signIn, signOut, auth } = NextAuth(async () => {
  ...
  return {
    ...
    providers: [
      ...
      // actually configured — same graceful-degradation pattern as every
      // other external integration in this app. Nothing configured = login
      // page just shows the email/password form, no broken Google button.
