...
const EXPORTS: Array<{
  ...
  { key: "loyalty_programs", label: "Rewards programme", direct: true, fetch: (t) => prisma.loyaltyProgram.findMany({ where: { tenantId: t } }) },
  { key: "loyalty_accounts", label: "Rewards members and balances", direct: true, fetch: (t) => prisma.loyaltyAccount.findMany({ where: { tenantId: t }, include: { entries: true } }) },
  { key: "recipes", label: "Recipes", direct: true, fetch: (t) => prisma.recipe.findMany({ where: { tenantId: t }, include: { components: true } }) },
  { key: "recipe_carries", label: "Part-used ingredients", direct: true, fetch: (t) => prisma.recipeCarry.findMany({ where: { tenantId: t } }) },
  { key: "outlets", label: "Trade map", direct: true, fetch: (t) => prisma.outlet.findMany({ where: { tenantId: t } }) },
  { key: "sales_routes", label: "Journey plans", direct: true, fetch: (t) => prisma.salesRoute.findMany({ where: { tenantId: t } }) },
  { key: "outlet_visits", label: "Shop visits", direct: true, fetch: (t) => prisma.outletVisit.findMany({ where: { tenantId: t }, take: 50000 }) },
  { key: "work_sites", label: "Work sites", direct: true, fetch: (t) => prisma.workSite.findMany({ where: { tenantId: t } }) },
  { key: "shifts", label: "Roster", direct: true, fetch: (t) => prisma.shift.findMany({ where: { tenantId: t }, take: 50000 }) },
  { key: "field_workers", label: "Casual workers", direct: true, fetch: (t) => prisma.fieldWorker.findMany({ where: { tenantId: t } }) },
  { key: "work_logs", label: "Casual work and pay", direct: true, fetch: (t) => prisma.workLog.findMany({ where: { tenantId: t }, take: 50000 }) },
  { key: "delivery_attempts", label: "Delivery attempts", direct: true, fetch: (t) => prisma.deliveryAttempt.findMany({ where: { tenantId: t }, take: 50000 }) },
  { key: "cod_settlements", label: "Rider cash settlements", direct: true, fetch: (t) => prisma.codSettlement.findMany({ where: { tenantId: t }, take: 20000 }) },
  { key: "party_bank_changes", label: "Supplier banking history", direct: true, fetch: (t) => prisma.partyBankChange.findMany({ where: { tenantId: t }, take: 20000 }) },
  { key: "stock_bins", label: "Warehouse bins", direct: true, fetch: (t) => prisma.stockBin.findMany({ where: { tenantId: t }, take: 20000 }) },
  { key: "stock_placements", label: "Where stock is", direct: true, fetch: (t) => prisma.stockPlacement.findMany({ where: { tenantId: t }, take: 50000 }) },
  { key: "product_serials", label: "Authentication codes", direct: true, fetch: (t) => prisma.productSerial.findMany({ where: { tenantId: t }, take: 50000 }) },
  { key: "product_serial_checks", label: "Authentication checks", direct: true, fetch: (t) => prisma.productSerialCheck.findMany({ where: { tenantId: t }, take: 50000 }) },
...
const EXCLUDED: Record<string, string> = {
  ...
  recipe_components:
    "Exported through their recipes — an ingredient line has no meaning away from the dish it belongs to.",
  loyalty_entries:
    "Exported through their accounts — every point movement is written out with the member whose balance it belongs to.",
