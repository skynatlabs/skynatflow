...
// to learn.
//
// Each column shows only its most recent 25 and a real count from a DB
// count() — a tenant with thousands of quotes (a real case we hit) used
// to load every single one into the page on every visit.
...
export default async function PipelinePage({
  ...
  const columns = await Promise.all(
    COLUMNS.map(async (col) => {
      const where = { tenantId, type: "QUOTE" as const, status: col.key };
      const [items, count, sum] = await Promise.all([
      ...
      return { ...col, items, count, amountCents: sum._sum.amountCents ?? 0 };
    })
  );

  const inPlay = (columns.find((c) => c.key === "DRAFT")?.amountCents ?? 0) +
        ...
        {columns.map((col) => (
          <div key={col.key} className={`kb-tile ${col.tint}`}>
            ...
            {col.count > COLUMN_LIMIT && (
              <Link
                href={`/dashboard/${tenantId}/quotes`}
                className="mt-3 block text-center text-xs font-semibold text-[var(--kb-text-dim)] hover:underline"
              >
                +{col.count - COLUMN_LIMIT} more — view all in Quotes &rarr;
              </Link>
            )}
          </div>
        ))}
